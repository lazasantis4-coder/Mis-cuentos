...
function cargarVideo(contenedor, id) {
    contenedor.innerHTML = `<iframe width="100%" height="250" src="https://www.youtube-nocookie.com/embed/${id}?autoplay=1&modestbranding=1&rel=0&iv_load_policy=3&fs=0&disablekb=1" frameborder="0" allow="autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
}

// Lógica de Instalación y Plan B
const btnInstalar = document.getElementById('btnInstalar');
const btnCopiar = document.getElementById('btnCopiar');
let deferredPrompt;

// 1. Botón de Copiar (Siempre funciona)
if(btnCopiar) {
    btnCopiar.addEventListener('click', () => {
        const enlace = window.location.href;
        navigator.clipboard.writeText(enlace).then(() => {
            alert("Lien copié ! Tu peux maintenant le coller dans tes notes ou l'envoyer à un ami.");
        });
    });
}

// 2. Botón de Instalar (Solo aparece si es compatible)
